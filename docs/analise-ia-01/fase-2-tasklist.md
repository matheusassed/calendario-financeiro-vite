...
### **3.1 Validação de Recorrência Inconsistente** ⭐ PRIORIDADE ALTA
- [ ] **3.1.1** Analisar `src/utils/recurrence.js` e identificar funções de validação ✅
- [ ] **3.1.2** Localizar função que valida regras de recorrência ✅
- [ ] **3.1.3** Adicionar validação de startDate < endDate ✅
- [ ] **3.1.4** Testar com recorrência inválida (startDate > endDate) ✅
- [ ] **3.1.5** Validar que recorrências válidas continuam funcionando ✅
- [ ] **3.1.6** Testar cenários edge cases (datas iguais, datas inválidas) ✅
- [ ] **3.1.7** Confirmar que erro é exibido corretamente na UI ✅
...
### **3.2 Tratamento de Timestamps Inconsistente** ⭐ PRIORIDADE ALTA
- [ ] **3.2.1** Analisar `src/hooks/useFirestoreQuery.js` e identificar conversões de Timestamp ✅
- [ ] **3.2.2** Localizar loop de conversão de dados do Firestore ✅
- [ ] **3.2.3** Adicionar try-catch para conversão de Timestamp ✅
- [ ] **3.2.4** Implementar logging de erro com logger padronizado ✅
- [ ] **3.2.5** Testar com dados válidos para confirmar que não quebra ✅
- [ ] **3.2.6** Testar com dados inválidos para validar tratamento de erro ✅
- [ ] **3.2.7** Verificar se logs aparecem corretamente no console ✅
...
### **3.3 Falta de Validação de Dados de Entrada** ⭐ PRIORIDADE MÉDIA
- [ ] **3.3.1** Analisar `src/utils/recurrence.js` e identificar funções sem validação ✅
- [ ] **3.3.2** Localizar função `getAffectedInstances` e outras funções críticas ✅
- [ ] **3.3.3** Adicionar validação para parâmetro `transaction` ✅
- [ ] **3.3.4** Adicionar validação para parâmetro `editOption` ✅
- [ ] **3.3.5** Adicionar validação para parâmetro `allTransactions` ✅
- [ ] **3.3.6** Implementar retornos seguros para dados inválidos ✅
- [ ] **3.3.7** Testar com dados inválidos para validar validações ✅
- [ ] **3.3.8** Confirmar que funcionalidades existentes continuam funcionando ✅
...
### **4.1 Console.log Direto em Produção** ⭐ PRIORIDADE MÉDIA
- [ ] **4.1.1** Executar busca em todo projeto por `console.log` ✅
- [ ] **4.1.2** Executar busca em todo projeto por `console.warn` ✅
- [ ] **4.1.3** Executar busca em todo projeto por `console.error` ✅
- [ ] **4.1.4** Identificar arquivos com console.log (mínimo 15 arquivos) ✅
- [ ] **4.1.5** Substituir `console.log` por `logger.info` ou remover ✅
- [ ] **4.1.6** Substituir `console.warn` por `logger.warn` ✅
- [ ] **4.1.7** Substituir `console.error` por `logger.error` ✅
- [ ] **4.1.8** Verificar se logger está sendo importado corretamente ✅
- [ ] **4.1.9** Testar se logs aparecem corretamente ✅
- [ ] **4.1.10** Confirmar que não quebra funcionalidades existentes ✅
...
### **4.2 Função formatFiscalMonth Duplicada** ⭐ PRIORIDADE BAIXA
- [ ] **4.2.1** Confirmar que `formatFiscalMonth` existe em `src/utils/helpers.js` ✅
- [ ] **4.2.2** Localizar função duplicada em `src/utils/recurrence.js` ✅
- [ ] **4.2.3** Remover função duplicada de `recurrence.js` ✅
- [ ] **4.2.4** Adicionar import de `formatFiscalMonth` em `recurrence.js` ✅
- [ ] **4.2.5** Verificar se não há outros arquivos usando a função duplicada ✅
- [ ] **4.2.6** Testar se funcionalidades de recorrência continuam funcionando ✅
- [ ] **4.2.7** Confirmar que build não quebra ✅
...
### **4.3 Imports Relativos Inconsistentes** ⭐ PRIORIDADE MÉDIA
- [ ] **4.3.1** Analisar estrutura de imports em todo projeto ✅
- [ ] **4.3.2** Identificar padrão de imports (relativos vs absolutos) ✅
- [ ] **4.3.3** Configurar path mapping no Vite (se necessário) ✅
- [ ] **4.3.4** Padronizar imports para usar caminhos absolutos ✅
- [ ] **4.3.5** Atualizar imports em `src/components/` ✅
- [ ] **4.3.6** Atualizar imports em `src/views/` ✅
- [ ] **4.3.7** Atualizar imports em `src/hooks/` ✅
- [ ] **4.3.8** Atualizar imports em `src/utils/` ✅
- [ ] **4.3.9** Verificar se build não quebra ✅
- [ ] **4.3.10** Testar funcionalidades principais para confirmar funcionamento ✅
